...
def hls_select(img, sthresh=(0, 255),lthresh=()):
    ...
    cv.imshow('BGR2HLS', hls_img)
    # 2) Apply a threshold to the S channel
    L = hls_img[:,:,1]
    S = hls_img[:,:,2]
    # 3) Return a binary image of threshold result
    binary_output = np.zeros_like(S)
    print('\nbinary_output Shape', binary_output.shape)
    binary_output[(S >= sthresh[0]) & (S <= sthresh[1])
                 & (L > lthresh[0]) & (L <= lthresh[1])] = 1
    #cv.imshow('binary output', binary_output)
...
def abs_sobel_thresh(img, orient='x', thresh=(0,255)):
    ...
    #cv.imshow(f'Sobel{orient}', scaled_sobel)
...
def mag_threshold(img, sobel_kernel=3, thresh=(0, 255)):
    ...
    #cv.imshow('binary_output', binary_output)
    return binary_output
...
def warp_image(img):
    ...
    print(int(source_points[0, 0]), int(source_points[0, 1]))
    img2 = cv.imread('test_images/test1.jpg')
    cv.circle(img2, (int(source_points[0, 0]), int(source_points[0, 1])), 5, (0, 255, 0), -1)
    cv.circle(img2, (int(source_points[1, 0]), int(source_points[1, 1])), 5, (0, 255, 0), -1)
    cv.circle(img2, (int(source_points[2, 0]), int(source_points[2, 1])), 5, (0, 255, 0), -1)
    cv.circle(img2, (int(source_points[3, 0]), int(source_points[3, 1])), 5, (0, 255, 0), -1)
    cv.imshow('Points', img2)
...
#                 ])
    ...
    cv.imshow('WarpedImg', warped_img)
...
def binary_pipeline(img):
    ...
    cv.imshow('GaussianBlur', img_copy)
    ...
    cv.imshow('final binary', final_binary)
...
def lane_fill_poly(binary_warped,undist,left_fit,right_fit, inverse_perspective_transform):
    ...
    # Draw the lane 
    #cv.fillPoly(color_warp, int_([pts]), (0,255, 0))
...
def image_pipeline():
    global window_search
    window_search = True
    print('WindowSearch', window_search)
